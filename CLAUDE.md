...
- There are no Claude Code hooks in this repository. Formatting, linting,
  ...
  it moved there.
- `merge-dependabot` — the one workflow skill shipped here, because landing bot
  PRs is the recurring task whose failure modes are specific to _this_
  toolchain (peer-range conflicts, the dependency cooldown, the packaging
  gates). Its survey script is read-only; every mutating step sits behind a
  single human approval. It is authored in `.agents/skills/merge-dependabot/`
  and mirrored into `.claude/skills/merge-dependabot/`, which Claude Code is
  the only tool that reads. The mirror is generated by `pnpm agents:sync` and
  committed: never hand-edit it, and never edit only one side. Drift fails
  `pnpm agents:check`, `tests/sync-agents.test.ts`, and lefthook's pre-commit
  hook. See AGENTS.md's "Repository automation (`.mjs`)" for why the
  copy is real files rather than a symlink.
