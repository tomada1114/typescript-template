...
| `managing-dependencies` | adding, bumping, or removing a package                    |
...
- Never weaken a gate to make a run pass: no lowered coverage threshold in
  ...
  security workflow, no removed `--frozen-lockfile`, no removed ESLint rule or
  `pnpm-workspace.yaml` supply-chain setting, no `@ts-ignore`, no blanket
